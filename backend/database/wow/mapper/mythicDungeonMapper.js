...
async function insertMythicDungeon(params) {
  ...
  return db.run(
    `INSERT INTO wow_mythic_dungeon(id, routes, ratings, utility_needs, enemy_tips, loot_pool)
     VALUES (?1, ?2, ?3, ?4, ?5, ?6)`,
...
async function getMythicDungeonList() {
  return db.all(
    `SELECT wow_mythic_dungeon.ratings,
            wow_mythic_dungeon.id,
            wow_dungeon.name_zh,
            wow_mythic_dungeon_tier.tier
     FROM wow_mythic_dungeon
              LEFT JOIN
...
async function getCurrentSeasonMythicDungeonList() {
  const idList = [505, 391, 503, 542, 378, 392, 499, 525];
  return db.all(
    `SELECT wow_mythic_dungeon.ratings,
            wow_mythic_dungeon.id,
            wow_dungeon.name_zh,
     ...
     FROM wow_mythic_dungeon
              LEFT JOIN wow_dungeon ON wow_mythic_dungeon.id = wow_dungeon.id
              LEFT JOIN wow_mythic_dungeon_tier on wow_mythic_dungeon.id = wow_mythic_dungeon_tier.id
     WHERE wow_mythic_dungeon.id IN (${idList.map(() => '?').join(',')})`,
    idList,
  );
}
